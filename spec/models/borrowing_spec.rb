...
RSpec.describe Borrowing, type: :model do
  ...
  describe 'validations' do
    context 'when initialized without dates (defaults)' do
      let(:borrowing) { described_class.new(user: create(:user), book: create(:book)) }

      before { borrowing.valid? }

      it 'sets default borrowed_at' do
        expect(borrowing.borrowed_at).to be_present
      end

      it 'sets default due_at' do
